...
3. 주의사항 (warnings): **${goal} 목표에 특별히 주의할 음식만** 1-2개 포함 (문자열 배열)
   ${goal === 'digestion' ? '- 소화 목표: 소화 난이도, 섭취 시간, 조리법 주의 (예: "기름기가 많아 소화에 부담될 수 있어요", "밤에 섭취하면 소화 시간이 길어져요")' : ''}
   ${goal === 'satiety' ? '- 포만감 목표: 포만감 방해 요소, 과다섭취 위험 (예: "포만감 없이 칼로리만 높아요", "빨리 먹으면 과식하기 쉬워요")' : ''}
   ${goal === 'energy' ? '- 에너지 목표: 혈당 급등락, 졸음 유발 요소 (예: "혈당 급등 후 피로가 올 수 있으니 천천히 드세요", "당분이 높아 에너지 급감을 유발할 수 있어요")' : ''}
   ${goal === 'weight' ? '- 체중 목표: 고열량, 과섭취 위험 (예: "칼로리가 높으니 양 조절이 중요해요", "GI 지수가 높아 체지방 축적이 쉬워요")' : ''}
   **중요: 모든 음식에 warnings를 달지 말고, 해당 목표에 정말 주의가 필요한 음식만 선별하세요**
...
다음 JSON 형식으로 응답해주세요:
{
  "foods": [
    ...
    },
    {
      ...
      "warnings": ["기름기가 많아 소화에 부담될 수 있어요", "밤에 섭취하면 소화 시간이 길어져요"]
    },
    {
      "name": "밥",
